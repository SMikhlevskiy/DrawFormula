...
public class FormulaDrawController {
	...
	public void drawGraphic(GraphicView graphicView, String textFormula, double xStart, double xEnd) {
		Log.i("Main activity.controller", "Start Draw Graphic");
	...
	public void readPreferences() {
		...
		Log.i("Main activity.Peferences", "Read xMin,xMax");
		...
		Log.i("Main activity.Peferences", "xMin=" + xMin);
		Log.i("Main activity.Peferences", "xMax=" + xMax);
		...
		for (int i = 0; i < FDConstants.colorSpinnerLines.length; i++) {
			...
			Log.i("Main activity.Peferences", "formula[" + i + "]=" + getFormulas(i));

			setdYdT(formulaDrawPreferences.getBoolean(FDConstants.APP_PREFERENCES_dYdT + i, false), i);
			Log.i("Main activity.Peferences", "dY/dT[" + i + "]=" + getdYdT(i));
